  ...
  dashboardBody(
    ...
    tabItems(
      ...
      tabItem(tabName='Home',
              fluidPage(
                titlePanel(title = span(img(src = "homephoto.png", height= 960), ""))),
              ),
      #Capacity 
      tabItem(tabName='Cap',
              tags$div(HTML("<b> <span style=font-size:250%;> DESCRIPTIVE STATISTICS </span> </b>")),
              fluidPage(
                fluidRow(
                  column(width=12,
                         height=500,
                         box(title = "Bus Fleet", status = "success", solidHeader = TRUE,
                              collapsible = TRUE,
                           width=6,
                           height=850,
                         ...
                         box(title = "Operater Wise Capacity ", status = "success", solidHeader = TRUE,
                             collapsible = TRUE,
                           width=6,
                           height=850,
                         ...
                         tags$div(HTML("<b> <span style=font-size:250%;>FORECAST </span> </b>")),
                         box( title = "Annual Ridership Forecast", status = "success", solidHeader = TRUE,
                              collapsible = TRUE,
                           width=6,
                             height=850,
                         ...
                         box( title = "Capacity Utilisation Forecast", status = "success", solidHeader = TRUE,
                              collapsible = TRUE,
                           width=6,
                             height=850,
              ...
              fluidPage(
                fluidRow(
                  column(width=12,
                         ...
                         box(
                           title = "Radius ", status = "success", solidHeader = TRUE,
                           collapsible = TRUE,
                           width=4,
                           height=350,
                         ...
                         box(title = "No. of Bus Stops within Radius", status = "success", solidHeader = TRUE,
                             collapsible = TRUE,
                           width=8,
                           height=350,
                         ...
                         box(title = "Bus Stops in Singapore ", status = "success", solidHeader = TRUE,
                             collapsible = TRUE,
                           width=12,
                           height=600,
                           leafletOutput("PlotMap", height = "540")
                         ),
                  )
                )
              ),
              ...
              fluidPage(
                fluidRow(
                  column(width=12,
                         ...
                         box(title = "Bus Fuel Type Distribution", status = "success", solidHeader = TRUE,
                             collapsible = TRUE,
                           width=3,
                             height=400,
                             tableOutput("tableFuel")
                         ),
                         box(title = "Bus type ", status = "success", solidHeader = TRUE,
                             collapsible = TRUE,
                           width=9,
                             height=400,
                             splitLayout(
                               ...
                               plotOutput("PlotPie1", height = 330),
                               plotOutput("PlotPie2", height = 330),
                               plotOutput("PlotPie3", height = 330),
                               plotOutput("PlotPie4", height = 330)
                             ),
                         ),
                         #Bus Age Graph
                         box(title = "Bus Age Distribution", status = "success", solidHeader = TRUE,
                             collapsible = TRUE,
                           width=12,
                             height=570,
                             plotlyOutput("BusAgeGraph", height = "520")
                         ),
                  )
                )
              ),
...
server <- function(input,output) {
  ...
  output$Plot1 <- renderPlot({
    q <- ggplot(data=Annual_Ridership_Adjusted, aes(x=factor(Year), y=Average.Annual.Ridership.Mils)) + geom_line(group = 1) +geom_point(colour = "forestgreen") + labs(x="Year",y="Average Annual Ridership (Millions)") + theme(axis.text = element_text(angle = 90, vjust = 0.5, size=15), axis.title = element_text( color="black", size=15, face=2), axis.line = element_line( colour = "black", linetype=1)) 
    print(q)
  })
  output$Table2 <- renderTable(Annual_Ridership, rownames = TRUE, colnames = TRUE)
  output$Plot2 <- renderPlot({
    q <- scatter.smooth(x=factor(Annual_Ridership_Adjusted$Year),y=Annual_Ridership_Adjusted$Average.Annual.Ridership.Mils, main = 'Annual Ridership Forecast',xlab = "Year", ylab = "Average Annual Ridership (Millions)") + theme(axis.text = element_text(angle = 90, vjust = 0.5, size=15), axis.title = element_text( color="black", size=15, face=2), axis.line = element_line( colour = "black", linetype=1))
    print(q)
  }) 
  #Plot 5
  output$Plot5 <- renderPlot({
    q <- ggplot(data=Operator_Capacity, aes(x=factor(Operator), y=Capacity)) + geom_bar(stat="identity", fill = "forestgreen", width = 0.5, size=0.8) + labs(x="Operator", y="Capacity (Millions)") + theme(axis.text = element_text(angle = 90, vjust = 0.5, size=15), axis.title = element_text( color="black", size=15, face=2), axis.line = element_line( colour = "black", linetype=1))
    print(q)
  }) 
  #Bus Fleet Table
  ...
  output$Plot3 <- renderPlot({
    ...
    q <- ggplot(data=Annual_Ridership_Forecast, aes(x=factor(Year), y=ARF)) + geom_line(group = 1) +geom_point(colour = "forestgreen", size = 5) + labs(x="Year",y="Average Annual Ridership Forecast (Millions)") + theme(axis.text = element_text(angle = 90, vjust = 0.5, size=15), axis.title = element_text( color="black", size=15, face=2), axis.line = element_line( colour = "black", linetype=1) )
    print(q)
  })
  #print(ARF)
  print(Capacity_Utilization_Forecast$TTS.Utilization)
  ...
  output$Plot4 <- renderPlot({
    ...
    print(CUF)
    q <- ggplot(data = Capacity_Utilization_Forecast ,aes(x=factor(Year), y=CUF)) + geom_line(group = 1) +geom_point(colour = "forestgreen", size = 5) + labs(x="Year",y="Annual Utilization Forecast %") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5)) + theme(axis.text = element_text(angle = 90, vjust = 0.5, size=15), axis.title = element_text( color="black", size=15, face=2), axis.line = element_line( colour = "black", linetype=1))
    print(q)
  })
  ##
  ...
  output$plot <- renderPlot({
    ...
    g+ geom_bar(stat = "sum", fill = "forestgreen", color = "forestgreen", width = 0.5, size=0.8) + labs(x="Number of MRT within the Radius", y="Counts") + theme(axis.text = element_text(angle = 90, vjust = 0.5, size=15), axis.title = element_text( color="black", size=15, face=2), axis.line = element_line( colour = "black", linetype=1))
  }) 
  output$tableFuel <- renderTable({
    q <- Bus_Fuel_Type
  })
  output$PlotPie1 <- renderPlot({
    q <- pie(SBST_FUEL, labels = SBST_E_SHARE, col = (c("red", "forest green")), main = "SBST Bus Fuel Type Share (%)")
    print(q)
  })
  output$PlotPie2 <- renderPlot({
    q <- pie(SMRT_FUEL, labels = SMRT_E_SHARE, col = (c("red", "forest green")), main = "SMRT Bus Fuel Type Share (%)") 
  })
  output$PlotPie3 <- renderPlot({
    pie(GAS_FUEL, labels = GAS_E_SHARE, col = (c("red", "forest green")), main = "GAS Bus Fuel Type Share (%)")
  })
  output$PlotPie4 <- renderPlot({
    pie(TTS_FUEL, labels = TTS_E_SHARE, col = (c("red", "forest green")), main = "TTS Bus Fuel Type Share (%)")
  })
}
